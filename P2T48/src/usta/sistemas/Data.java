package usta.sistemas;

import java.io.BufferedWriter;
import java.io.File;
import java.io.FileWriter;
import java.util.Scanner;

public class Data {
    public static boolean Data(String name, String lname, String faculty) {
        boolean ret;
        try {
            File file = new File("D:/estudiante.txt");
            if (file.exists()) {
                FileWriter file2 = new FileWriter(file, true);
                BufferedWriter data = new BufferedWriter(file2);
                data.write("\n" + name + " | " + lname + " | " + faculty);
                data.close();
                ret = true;
            } else {
                ret = false;
            }
            return ret;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    }
    public static String[][] all_data() {
        String[][] students;
        String texto, tmp;
        int row = 0, lines = 0, limit1, limit2;
        try {
            File file = new File("D:/estudiante.txt");
            if (file.exists()) {
                Scanner file2 = new Scanner(file);
                while (file2.hasNextLine()) {
                    lines++;
                    file2.nextLine();
                }
                file2.close();
                file2 = null;
                students = new String[lines][3];
                file2 = new Scanner(file);
                while (file2.hasNextLine()) {
                    texto = file2.nextLine();
                    limit1 = texto.indexOf("|");
                    if (limit1 != -1) {
                        students[row][0] = texto.substring(0, limit1);
                        tmp = texto.substring(limit1 + 1);
                        limit2 = tmp.indexOf("|");
                        students[row][1] = tmp.substring(0, limit2);
                        students[row][2] = tmp.substring(limit2 + 1);
                        row++;
                    }
                }
                file2.close();
                return students;
            } else {
                return null;
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    }
}
